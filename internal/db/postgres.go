...
func (c *Client) ensureRuntimeSchema(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS taste_profile_summary (
			scope TEXT PRIMARY KEY,
			total_plays INTEGER NOT NULL DEFAULT 0,
			distinct_played_tracks INTEGER NOT NULL DEFAULT 0,
			distinct_played_artists INTEGER NOT NULL DEFAULT 0,
			rated_tracks INTEGER NOT NULL DEFAULT 0,
			replay_affinity_score DOUBLE PRECISION NOT NULL DEFAULT 0,
			novelty_tolerance_score DOUBLE PRECISION NOT NULL DEFAULT 0,
			updated_at TIMESTAMP NOT NULL
		)`,
		`CREATE TABLE IF NOT EXISTS taste_profile_artist_features (
			scope TEXT NOT NULL,
			...
			updated_at TIMESTAMP NOT NULL,
			PRIMARY KEY (scope, artist_name)
		)`,
		`CREATE TABLE IF NOT EXISTS taste_profile_album_features (
			scope TEXT NOT NULL,
			...
			overexposure_score DOUBLE PRECISION NOT NULL DEFAULT 0,
			updated_at TIMESTAMP NOT NULL,
			PRIMARY KEY (scope, album_id)
		)`,
		...
		`CREATE INDEX IF NOT EXISTS idx_taste_profile_artist_features_familiarity ON taste_profile_artist_features(scope, familiarity_score DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_taste_profile_artist_features_fatigue ON taste_profile_artist_features(scope, fatigue_score DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_taste_profile_album_features_overexposure ON taste_profile_album_features(scope, overexposure_score DESC)`,
		...
		`ALTER TABLE albums ADD COLUMN IF NOT EXISTS rating INTEGER DEFAULT 0`,
